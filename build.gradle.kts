    ...
    resolutionStrategy.eachDependency {
      ...
      // We want to force all support libraries to use the same version, even if they"re transitive.
        requested.group == "com.android.support" &&
            !requested.name.contains("multidex") -> useVersion(versions.support)
      // We want to force all play services libraries to use the same version, even if they"re transitive.
        requested.group == "com.google.android.gms" -> useVersion(versions.playServices)
      // We want to force all play services libraries to use the same version, even if they"re transitive.
        requested.group == "com.google.firebase" -> useVersion(versions.firebase)
