  ...
  SubcomposeLayout { constraints ->
    ...
    layout(layoutWidth, layoutHeight) {
      val topBarPlaceables =
        subcompose(ScaffoldLayoutContent.TopBar, topBar).map { it.measure(looseConstraints) }

      val contentInsets =
        incomingContentPadding + contentWindowInsets.asPaddingValues(this@SubcomposeLayout)
        ...
        subcompose(ScaffoldLayoutContent.MainContent) {
            val innerPadding =
              PaddingValues(
                ...
                start =
                  contentInsets.calculateStartPadding((this@SubcomposeLayout).layoutDirection),
      ...
      bottomBarPlaceables.forEach { it.place(0, layoutHeight - (bottomBarHeight ?: 0)) }
      // Explicitly not using placeRelative here as `leftOffset` already accounts for RTL
      fabPlacement?.let { placement ->
        fabPlaceables.forEach { it.place(placement.left, layoutHeight - fabOffsetFromBottom!!) }
      }
    }
  }
}
