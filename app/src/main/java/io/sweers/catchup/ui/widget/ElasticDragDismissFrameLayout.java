...
public class ElasticDragDismissFrameLayout extends FrameLayout {
  ...
  float dragDismissDistance = Float.MAX_VALUE;
  float dragDismissFraction = -1f;
  float dragDismissScale = 1f;
  boolean shouldScale = false;
  float dragElacticity = 0.8f;
  ...
  @Override public void onNestedScroll(View target,
      int dxConsumed,
      int dyConsumed,
      int dxUnconsumed,
      int dyUnconsumed) {
  ...
  public static class SystemChromeFader extends ElasticDragDismissCallback {
    ...
    public SystemChromeFader(Activity activity) {
      ...
      statusBarAlpha = Color.alpha(activity.getWindow()
          .getStatusBarColor());
      navBarAlpha = Color.alpha(activity.getWindow()
          .getNavigationBarColor());
      fadeNavBar = UiUtil.isNavBarOnBottom(activity);
    }

    @Override public void onDrag(float elasticOffset,
        float elasticOffsetPixels,
        float rawOffset,
        float rawOffsetPixels) {
      if (elasticOffsetPixels > 0) {
        ...
        activity.getWindow()
            .setStatusBarColor(ColorUtils.modifyAlpha(activity.getWindow()
        ...
        activity.getWindow()
            .setNavigationBarColor(ColorUtils.modifyAlpha(activity.getWindow()
